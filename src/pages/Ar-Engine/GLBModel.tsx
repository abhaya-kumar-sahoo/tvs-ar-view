...
const GLBModel = (
  ...
  return new Promise((resolve, reject) => {
    const envMap = new THREE.CubeTextureLoader()
      .setPath("/env/") // folder must have px.jpg, nx.jpg, py.jpg, ny.jpg, pz.jpg, nz.jpg
      .load(["px.jpg", "nx.jpg", "py.jpg", "ny.jpg", "pz.jpg", "nz.jpg"]);
      ...
      (gltf: GLTF) => {
        ...
        model.traverse((child: any) => {
          if (child.isMesh && child.material) {
            ...
            const baseColor = child.material.color;
            if (baseColor.r < 0.1 && baseColor.g < 0.1 && baseColor.b < 0.1) {
              child.material.color.setRGB(0.6, 0.6, 0.6);
            }
        ...
        resolve(model); // ✅ resolves when model is ready
      },
      undefined,
      (error) => {
        console.error("GLB Model load failed:", error);
